...
/home/stanley/nod/iree-build-notrace/compiler/bindings/python/iree/compiler/tools/../_mlir_libs/iree-compile torch_example_transform.mlir --iree-input-type=auto --iree-vm-bytecode-module-output-format=flatbuffer-binary --iree-hal-target-backends=rocm --mlir-print-op-on-diagnostic=false --iree-input-type=torch --mlir-print-op-on-diagnostic=false --iree-llvmcpu-target-cpu-features=host --iree-llvmcpu-target-triple=x86_64-linux-gnu --iree-stream-resource-index-bits=64 --iree-vm-target-index-bits=64 --iree-rocm-target-chip=gfx1100 --iree-vm-bytecode-module-strip-source-map=true --iree-opt-strip-assertions=true --iree-vm-target-truncate-unsupported-floats --iree-codegen-llvmgpu-enable-transform-dialect-jit=false --iree-preprocessing-transform-spec-filename=/home/stanley/nod/macroHipKernel/embedded_example_transform_spec.mlir -o torch_argmax.vmfb
