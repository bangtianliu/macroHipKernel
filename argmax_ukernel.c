...
// TODO: Change FN signature to match application.
extern "C" __global__ void argmax_F32I32(float *inputBuffer,
  ...
  if (laneID == 0)
    outputBuffer[output_offset] = indexVal;
}
...
extern "C" __global__ void argmax_F32I64(float *inputBuffer,
  ...
  // if there are multiple max value holder, find smallest index (argmax
  // semantics).
  int64_t indexVal = wgMax == laneMax ? laneResult : __INT32_MAX__;
  for (int i = 1; i < warpSize; i *= 2) {
    indexVal = min(__shfl_xor(indexVal, i), indexVal);
  }
  if (laneID == 0)
...
extern "C" __global__ void argmax_F16I32(half *inputBuffer,
                                                        int32_t *outputBuffer,
                                                        size_t* reductionSize) {
...
// TODO: Change FN signature to match application.
extern "C" __global__ void argmax_F16I64(half *inputBuffer,
                                                        size_t input_offset,
                                                        int64_t *outputBuffer,
                                                        size_t output_offset,
                                                        size_t reductionSize) {
